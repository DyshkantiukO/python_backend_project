...
from myapplication.models import User, Category, Record
...
def get_delete_record(record_id):
    ...
    if request.method == 'GET':
        return jsonify(id=record.record_id, user_id=record.user_id, category_id=record.category_id, created_at=record.created_at, amount=record.amount)
    elif request.method == 'DELETE':
